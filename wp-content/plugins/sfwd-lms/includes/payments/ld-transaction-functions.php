 ...
 *
 * @return array $meta Array of transaction meta data
 */
function learndash_transaction_get_payment_meta( $transaction_id = 0 ): array {
	...
	if ( learndash_get_post_type_slug( 'transaction' ) !== $transaction->post_type ) {
		return array();
	}
 ...
 * @return array $meta Array of transaction meta data
 */
function learndash_transaction_get_coupon_meta( $transaction_id = 0 ): array {
	$transaction_id = absint( $transaction_id );
	if ( empty( $transaction_id ) ) {
		return array();
	}
	...
	$final_price = learndash_transaction_get_final_price( $transaction_id );
	...
	if ( ! empty( $coupon_meta ) ) {
		...
		$discount = ( 'flat' === $coupon_meta['type'] ? learndash_get_price_formatted( $coupon_meta['discount'], $currency ) : $coupon_meta['amount'] . '%' );
 ...
 *
 * @return int $final_price Actual price paid for transaction
 */
function learndash_transaction_get_final_price( $transaction_id ): int {
	$transaction_id = absint( $transaction_id );
	if ( empty( $transaction_id ) ) {
		return array();
	}
 ...
 * @return array $meta Array of transaction meta data
 */
function learndash_transaction_get_stripe_meta( $transaction_id = 0 ): array {
	$transaction_id = absint( $transaction_id );
	if ( empty( $transaction_id ) ) {
		return array();
	}
 ...
 * @return array $meta Array of transaction meta data
 */
function learndash_transaction_get_paypal_meta( $transaction_id = 0 ): array {
	$transaction_id = absint( $transaction_id );
	if ( empty( $transaction_id ) ) {
		return array();
	}
	...
	if ( 'paypal' === $transaction_processor ) {
 ...
 * @return array $meta Array of transaction meta data
 */
function learndash_transaction_get_razorpay_meta( $transaction_id = 0 ): array {
	$transaction_id = absint( $transaction_id );
	if ( empty( $transaction_id ) ) {
		return array();
	}
	...
	if ( 'razorpay' === $transaction_processor ) {
...
function learndash_transaction_create( array $meta_fields, WP_Post $post, WP_User $user ): void {
	...
	if ( $transaction_id > 0 && ! is_wp_error( $transaction_id ) ) {
		/**
		...
		learndash_send_purchase_invoice_email( $user->ID, $transaction_id );
	}
}
