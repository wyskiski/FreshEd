 ...
 * @param string $headers Addtional email headers.
 * @param array  $attachments Email attachments.
 */
function learndash_emails_send( $user_email = '', $email_args = array(), $headers = '', $attachments = '' ) {
	if ( empty( $user_email ) ) {
		return false;
	}
 ...
 */
function learndash_send_purchase_invoice_email( $user_id = 0, $transaction_id = 0 ) {
	$user_id = absint( $user_id );
	...
	if ( 'on' === $email_setting['enabled'] ) {
		...
		if ( ! in_array( $purchased_post->post_type, learndash_get_post_type_slug( array( 'course', 'group' ) ), true ) ) {
			return false;
		}

		if ( learndash_get_post_type_slug( 'transaction' ) !== $transaction_post->post_type ) {
			return false;
		}
		...
		$attachments = array( $pdf_data['filepath'] . $pdf_data['filename'] );

		if ( ( ! empty( $email_setting['subject'] ) ) && ( true === $pdf ) ) {
			learndash_emails_send( $user->user_email, $email_setting, $headers, $attachments );
